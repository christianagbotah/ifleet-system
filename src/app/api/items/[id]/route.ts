...
// DELETE /api/items/[id] — Soft delete or hard delete
export async function DELETE(
  ...
  try {
    ...
    if (hardDelete) {
      ...
      return NextResponse.json({ success: true, id, message: 'Item permanently deleted' })
    }

    // Soft delete — deactivate
    if (!existing.isActive) {
      return NextResponse.json({ error: 'Item is already inactive' }, { status: 400 })
    }
