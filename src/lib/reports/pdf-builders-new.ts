...
export async function buildTyreReportPdf(params: ReportParams): Promise<jsPDF> {
  ...
  const headers = ['Serial #', 'Truck', 'Brand', 'Purchase Date', `Price (${CEDI})`, 'Condition', 'Last Inspection', 'Status']
...
export async function buildInsuranceClaimsReportPdf(params: ReportParams): Promise<jsPDF> {
  ...
  const headers = ['Claim #', 'Policy #', 'Provider', 'Truck', 'Type', 'Incident Date', 'Location', `Claimed (${CEDI})`, `Approved (${CEDI})`, 'Status', 'Submitted']
...
export async function buildWarehouseReportPdf(params: ReportParams): Promise<jsPDF> {
  ...
  const headers = ['SKU', 'Name', 'Category', 'Quantity', 'Min Stock', `Unit Price (${CEDI})`, `Total Value (${CEDI})`, 'Unit', 'Warehouse', 'Supplier', 'Status', 'Last Restocked', 'Expiry Date']
...
export async function buildDriverIncentivesReportPdf(params: ReportParams): Promise<jsPDF> {
  ...
  const headers = ['Driver', 'Type', 'Title', 'Period', `Amount (${CEDI})`, 'Status', 'Approved By', 'Approved Date', 'Paid Date', 'Metric']
...
export async function buildTollReportPdf(params: ReportParams): Promise<jsPDF> {
  ...
  const headers = ['Date', 'Truck', 'Driver', 'Trip #', 'Toll Point', 'Toll Type', 'Route', `Amount (${CEDI})`, 'Payment', 'Direction', 'Overloaded', 'Status']
...
export async function buildCashAdvancesReportPdf(params: ReportParams): Promise<jsPDF> {
  ...
  const headers = ['Request Date', 'Driver', 'Employee ID', 'Trip #', 'Purpose', `Amount (${CEDI})`, 'Payment', 'Status', 'Approved By', 'Approved At', 'Disbursed At', 'Deducted', 'Remaining']
...
export async function buildDailySummaryPdf(params: ReportParams): Promise<jsPDF> {
  ...
  // Expenses table
  const expHeaders = ['Date', 'Truck', 'Category', 'Description', `Amount (${CEDI})`, 'Payment', 'Reference']
...
export async function buildBorderCrossingsReportPdf(params: ReportParams): Promise<jsPDF> {
  ...
  const headers = ['Truck', 'Driver', 'Trip #', 'Border', 'Country', 'Direction', 'Status', 'Queued At', 'Cleared At', 'Wait (min)', `Fee (${CEDI})`, 'Doc Status', 'Notes']
...
export async function buildLoadBoardReportPdf(params: ReportParams): Promise<jsPDF> {
  ...
  const headers = ['Date', 'Client', 'Pickup', 'Dropoff', 'Commodity', 'Weight', 'Truck Type', `Rate (${CEDI})`, 'Budget', 'Status', 'Assigned Truck', 'Assigned Driver', 'Contact']
...
export async function buildFuelAnomalyReportPdf(params: ReportParams): Promise<jsPDF> {
  ...
  const headers = ['Date', 'Truck', 'Trip #', 'Odometer', 'Liters', `Cost (${CEDI})`, 'Cost/Liter', 'L/100km', 'Cost/km', 'Station', 'Status']
...
export async function buildCostAnalyticsReportPdf(params: ReportParams): Promise<jsPDF> {
  ...
  const headers = ['Truck', 'Make', `Fuel (${CEDI})`, `Maint (${CEDI})`, `Other (${CEDI})`, `Total (${CEDI})`, 'Distance (km)', 'Tonnes', 'Cost/km', 'Cost/Ton']
...
export async function buildFuelAnalyticsReportPdf(params: ReportParams): Promise<jsPDF> {
  ...
  const truckHeaders = ['Truck', 'Liters', `Cost (${CEDI})`, 'Avg Cost/L', 'Fill-ups', 'Avg Fill (L)']
  ...
  if (stationRows.length > 0) {
    const stationHeaders = ['Station', 'Liters', `Cost (${CEDI})`, 'Avg Cost/L', 'Transactions']
