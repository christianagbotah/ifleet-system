...
export function DriverDetailSheet({ open, onOpenChange, driverId }: DriverDetailSheetProps) {
  ...
  const getTitle = (): React.ReactNode => {
    if (loading || error || !driver) return 'Driver Details'
    return (
  ...
  const getDescription = (): string | undefined => {
    if (loading) return 'Loading driver information...'
    if (error) return 'Error loading driver information'
    if (driver) return 'Driver profile & activity details'
    return undefined
  }
