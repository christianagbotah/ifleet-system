...
import {
  ...
  Loader2, ChevronDown, ChevronLeft, ChevronRight, Eye, Ban,
  AlertTriangle, TrendingUp, Banknote, Truck, Route, CalendarDays,
  MessageSquare, Percent, X, User, Star, AlertCircle,
} from 'lucide-react'
...
function SubmitForApprovalDialog({
  open,
  onOpenChange,
  onSubmitted,
}: {
  ...
  const [expenses, setExpenses] = React.useState<PendingExpense[]>([])
  const [selectedId, setSelectedId] = React.useState('')
  const [approvalLevel, setApprovalLevel] = React.useState('1')
  const [notes, setNotes] = React.useState('')
  const [submitting, setSubmitting] = React.useState(false)
  ...
  React.useEffect(() => {
    if (open) {
      ...
      setApprovalLevel('1')
      setNotes('')
      fetchExpenses({ status: 'approved', limit: 50 })
        .then(res => {
          // Filter to expenses that don't already have an approval
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Select an expense to submit for the approval workflow.
          </DialogDescription>
        </DialogHeader>
            ...
            <>
                ...
                >
                  ...
                  {expenses.map(e => (
                    <option key={e.id} value={e.id}>
                      {e.truck.plateNumber} — {e.category} — {formatCurrency(e.amount)} — {formatDate(e.date)}
                    </option>
                  ))}
                </select>
              </div>
              ...
              {selected && (
                ...
                >
                  ...
                  <p className="font-bold text-amber-600">{formatCurrency(selected.amount)}</p>
                </motion.div>
              )}
...
export function ExpenseApprovalsView() {
  ...
  const filteredApprovals = searchQuery
    ? approvals.filter(a =>
        a.expense.truck.plateNumber.toLowerCase().includes(searchQuery.toLowerCase()) ||
        a.expense.category.toLowerCase().includes(searchQuery.toLowerCase()) ||
        a.expense.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
        a.requestedBy.name.toLowerCase().includes(searchQuery.toLowerCase())
      )
    : approvals
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Manage expense approval workflow and multi-level authorization</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => { setSearchQuery(''); setActiveTab('all'); setPage(1) }}>
        ...
        {loading ? (
          ...
          <>
            ...
            <StatsCard
              ...
              value={`${formatCurrency(summary?.approvedThisMonthAmount || 0)}`}
              changeLabel={`${summary?.approvedThisMonthCount || 0} approved`}
              className="cursor-default"
            />
            <StatsCard
              icon={TrendingUp}
              ...
              changeLabel={`${formatCurrency(summary?.pendingAmount || 0)} value`}
              className="cursor-default"
            />
            <StatsCard
              icon={Banknote}
              title="Avg Approval Time"
              value={`${summary?.avgApprovalHours || 0}h`}
              changeLabel={`${summary?.totalCount || 0} total processed`}
              className="cursor-default"
            />
          </>
        )}
        ...
        {loading ? (
          ...
          <div className="p-8">
            <EmptyState
              ...
              description={activeTab !== 'all' ? `No ${activeTab} approvals` : 'No approvals yet. Submit expenses for approval from the Expenses page.'}
            />
          </div>
        ) : (
          <>
                    ...
                    <TableRow key={approval.id}>
                      <TableCell>
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground max-w-[180px] truncate">{approval.expense.description}</p>
                        </div>
                      </TableCell>
                      <TableCell>
                        <span className="font-medium text-sm">{approval.expense.truck.plateNumber}</span>
                      </TableCell>
                      <TableCell>
                        <div>
                          ...
                          {approval.approvedAmount !== null && approval.approvedAmount < approval.amount && (
                            <p className="text-xs text-orange-600">→ {formatCurrency(approval.approvedAmount)}</p>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                        <span className="text-xs text-muted-foreground">{getApprovalLevelLabel(approval.approvalLevel)}</span>
                      </TableCell>
                      <TableCell>
                        <Badge className={STATUS_CONFIG[approval.status]?.color || ''} variant="outline">
                          {STATUS_CONFIG[approval.status]?.label || approval.status}
      ...
      {/* Dialogs */}
      <ApprovalDialog
        open={dialogAction === 'approve' ? approvalDialogOpen : dialogAction === 'partial' ? approvalDialogOpen : false}
        onOpenChange={(open) => {
          if (dialogAction === 'approve' || dialogAction === 'partial') {
      ...
      <SubmitForApprovalDialog
        ...
        onSubmitted={() => { setPage(1); loadData() }}
      />
      <ApprovalDetailSheet
